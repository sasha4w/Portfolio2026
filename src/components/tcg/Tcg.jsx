  ...
  return (
    ...
    >
      <g stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
        <g transform="translate(-103.000000, -362.000000)" fill="currentColor">
...
// --- DONNÉES DU SLIDER ---
// → Remplace les `src` par tes vraies screenshots
const SLIDES = [
  {
    src: "/images/homepage.png",
    label: "Home · Accueil",
    caption: "Hub principal — quêtes, boutique et ouverture de boosters",
  },
  {
    src: "/images/deckbuilder.png",
    label: "Deck Builder",
    caption: "Construction de deck avec gestion des slots en temps réel",
  },
  {
    src: "/images/fight.png",
    label: "Combat",
    caption: "Interface de combat en ligne avec effets et animations",
  },
  {
    src: "/images/marketplace.png",
    label: "Marketplace",
    caption: "Vente et achat de cartes entre joueurs avec historique",
  },
];
...
function AppSlider() {
  ...
  useEffect(() => {
    if (paused) return;
    const t = setInterval(() => goTo(current + 1, "next"), 4000);
  ...
  return (
    ...
    >
      ...
      <div class={styles.sliderTrack}>
        {SLIDES.map((slide, i) => {
          ...
          if (i === current) {
            ...
            slideClass = styles.slideHidden;
          }

          return (
            <div key={i} class={`${styles.slide} ${slideClass}`}>
              <img
                src={slide.src}
                alt={slide.label}
      ...
      {/* Caption */}
      <div class={styles.sliderCaption}>
        <span class={styles.captionLabel}>{SLIDES[current].label}</span>
        <span class={styles.captionText}>{SLIDES[current].caption}</span>
      </div>

      {/* Flèches */}
      <button
        class={`${styles.sliderArrow} ${styles.sliderArrowLeft}`}
        onClick={prev}
      ...
      <button
        class={`${styles.sliderArrow} ${styles.sliderArrowRight}`}
        onClick={next}
      ...
      <div class={styles.sliderDots}>
        {SLIDES.map((_, i) => (
          <button
            key={i}
            class={`${styles.dot} ${i === current ? styles.dotActive : ""}`}
      ...
      {!paused && (
        <div class={styles.progressBar} key={current}>
    ...
    <section id="tcg">
      <div class="container">
        ...
        <Reveal delay="d2">
          <div class={styles.tcgRepoLinks}>
            <a
              ...
              target="_blank"
            >
              <GithubIcon /> Frontend (React + Vite)
            </a>
            <a
        ...
        <div class={styles.tcgPillars}>
          <Reveal>
            <div class={`${styles.pillar} ${styles.artPillar}`}>
              ...
              <ul class={styles.pillarList}>
                <li>
                  Univers thématique : chaque archétype évoque une ambiance
                  visuelle spécifique
                </li>
                <li>
                  3 piliers complémentaires pour une expérience de jeu complète
                </li>
                <li>Collection et ouverture de boosters</li>
                <li>Économie interne avec vente et achat de cartes</li>
                <li>Construction de deck pour des combats en ligne</li>
              </ul>
            </div>
          </Reveal>

          <Reveal delay="d1">
            <div class={`${styles.pillar} ${styles.techPillar}`}>
              <div class={styles.pillarIcon}>
                <SettingsIcon />
              </div>
